	...
	public static int[] Q2()	// Possible offsprings and expected ratios
	{
		...
		while (true)
		{
			...
			*/
			...
			/*
			Console.WriteLine("Parents' genotypes: {0} x {1}", genotype[0], genotype[1]);		// TEST
			Console.WriteLine("bigSolArray" + "\n-------------------");
			for (int i=0; i<Math.Pow(3, numTraits); i++)
			{
				Console.WriteLine("{0, -12} | {1}", bigSolArray[i, 0], bigSolArray[i, 1]);
			}

			Console.WriteLine("\ntrimSolArray" + "\n-------------------");		// TEST
			for (int i=0; i<(trimSolArray.Length/2); i++)
			{
				Console.WriteLine("{0, -12} | {1}", trimSolArray[i, 0], trimSolArray[i, 1]);
			}
			Console.ReadLine();
			*/
			...
			Console.WriteLine(solFORMAT, "Genotype", "Expected Proportion" + 
				"\n-------------|--------------------");
			...
			for (int i=0; i<(trimSolArray.Length/2); i++)
			{
				...
				{
					if (trimSolArray[i, 0] == ansArray[j, 0] || trimSolArray[i, 1] == ansArray[j, 1])
						count++;
				}
				if (count == 0)
					correct = false;
			...
			for (int i=0; i<notBlank; i++)
			{
				...
				{
					if (trimSolArray[j, 0] == ansArray[i, 0] || trimSolArray[j, 0] == ansArray[i, 0])
						count++;
				}
				if (count == 0)
					correct = false;
